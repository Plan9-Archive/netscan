...
scanner(void *)
{
	...
	while(recv(cin, &p) > 0){
		...
		threadsetname("scanner %s!%s!%s", s->proto, s->host, s->service);
...
threadmain(int argc, char *argv[])
{
	...
	if(argc == 1){
		...
		/* try to resolve named services through cs if not integer */
		for(i = 0; i < argc-1; i++){
			if(atoi(argv[i+1]) != 0)
				theports[i] = strdup(argv[i+1]);
			else
				theports[i] = csgetvalue(netmtpt, "tcp", argv[i+1], "port", nil);

			if(theports[i] == nil)
				sysfatal("resolving port %s failed: %r", argv[i+1]);
		}
	}
	...
	cin = chancreate(sizeof(scan*), nio);
	cout = chancreate(sizeof(scan*), nio);
